...
- `:::author` 〜 `:::` = 「筆者」コメントボックス
- `:::warn` 〜 `:::` = 赤系警告ボックス
- `:::code` 〜 `:::` = 招待コードのコピーボタン付きボックス。1行1件で `アプリ名|コード|補足(任意)`。
  コピー時に GA4 の `copy_click` イベント（app / code）を送る（lp1と同じ計測思想）
...
記事ページに Article + BreadcrumbList のJSON-LDを自動出力。著者=オリパ研究家 ユウジ（Person）、発行者=オリパ情報局（Organization）。
